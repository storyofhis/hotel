...
const ReservationCoupon: React.FC<TypeReservationCouponProps> = (props: TypeReservationCouponProps) => {
  ...
  const renderMessage = () => {
    if (coupon.discountAmmount && !coupon.error) {
      return <p className={styleClasses["reservation-details__coupon__success"]}>{coupon.discountAmmount}% discount applied successfully.</p>;
    }
  ...
  return (
    <li className={styleClasses["reservation-details__coupon"]}>
      <div className={styleClasses["reservation-details__coupon__content"]}>
        ...
        />
        <Button type="button" onClick={applyHandler} disabled={inputValue.length < 3} loading={coupon.status === "pending"}>
          Apply
        </Button>
      </div>
