...
const ReservationDetails: React.FC<TypeReservationDetailsProps> = (props: TypeReservationDetailsProps) => {
  ...
  const renderDetailsItem = (key: TypeDetail) => {
    ...
    const items: any = {
      ...
      room: !!roomDetails && roomDetails.type ? <ReservationDetailsItem title="Room" value={roomDetails.type.title} /> : null,
      view: !!roomDetails && roomDetails.view ? <ReservationDetailsItem title="View" value={roomDetails.view.title} /> : null,
  ...
  return (
    <div className={elClasses.join(" ")}>
      <h2 className={styleClasses["reservation-details__title"]}>
        {!!selectedHotel ? selectedHotel.name : ""} {selectedHotel?.details?.city && <span>{`(${selectedHotel.details.city})`}</span>}
      </h2>
