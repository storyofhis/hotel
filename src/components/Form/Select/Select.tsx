...
const Select: React.FC<TypeSelectProps> = ({ options, id, label, onChange, value, validity, validators, isTouched, validationMessage, ...rest }: TypeSelectProps) => {
  ...
  return (
    <div className={wrapperClasses.join(" ")}>
      ...
      <ReactSelect options={options} inputId={id} instanceId={id} name={id} classNamePrefix="select" onChange={handleChange} value={createOption(value, options)} {...rest} />
      <div className={formClasses["form__info"]}>
        <div className={formClasses["form__info__content"]}>{validity === false && isTouched && !!validationMessage && <p className={formClasses["form__info__invalid-text"]}>{validationMessage}</p>}</div>
      </div>
    </div>
  );
};
