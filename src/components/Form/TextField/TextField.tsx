...
const TextField = React.forwardRef<HTMLInputElement, TypeTextFieldProps>(
  ({ id, onChange, value, validity, validators, isTouched, type, placeholder, autocomplete, label, validationMessage, disabled, disabledMessage }: TypeTextFieldProps, ref) => {
    ...
    return (
      <div className={wrapperClasses.join(" ")}>
        ...
        <div className={inputClasses["input__inner-wrapper"]}>
          {type === "textarea" ? (
            <textarea id={id} name={id} className={inputClasses["input__field"]} rows={3} onChange={changeHandler} value={value} {...additionalAttrs} />
          ) : (
            <input id={id} name={id} className={inputClasses["input__field"]} type={type} placeholder={placeholder} autoComplete={autocomplete} onChange={changeHandler} value={value} {...additionalAttrs} />
          )}
        </div>

        <div className={formClasses["form__info"]}>
          <div className={formClasses["form__info__content"]}>
            {validity === false && isTouched && validationMessage && <p className={formClasses["form__info__invalid-text"]}>{validationMessage}</p>}
            {additionalAttrs["disabled"] === true && <p className={formClasses["form__info__warning-text"]}>{disabledMessage}</p>}
          </div>
        </div>
      </div>
    );
