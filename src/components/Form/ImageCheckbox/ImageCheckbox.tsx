...
const ImageCheckbox: React.FC<TypeImageCheckboxProps> = ({ id, onChange, value, validity, validators, isTouched, label, validationMessage, options }: TypeImageCheckboxProps) => {
    ...
    <>
      <div className={wrapperClasses.join(" ")}>
          ...
          {options.length > 0 &&
            options.map((option: TypeOption, index) => (
              <div className={checkboxClasses["image-checkbox"]} key={index}>
                <input type="radio" id={option.id} name={id} className={checkboxClasses["image-checkbox__field"]} onChange={changeHandler} value={option.id} checked={option.id === value} />
                <label htmlFor={option.id} className={checkboxClasses["image-checkbox__label"]}>
                  ...
                  {option.imgUrl && (
                    <div className={checkboxClasses["image-checkbox__label__img-wrapper"]}>
                      <img src={option.imgUrl} alt={option.label} className={checkboxClasses["image-checkbox__label__img"]} />
                    </div>
                  )}
                  {option.meta && <p className={checkboxClasses["image-checkbox__label__meta"]}>{option.meta}</p>}
                </label>
              </div>
            ))}
        </div>
        ...
        <div className={formClasses["form__info"]}>
          <div className={formClasses["form__info__content"]}>{validity === false && isTouched && !!validationMessage && <p className={formClasses["form__info__invalid-text"]}>{!!validationMessage && validationMessage}</p>}</div>
        </div>
      </div>
    </>
  );
