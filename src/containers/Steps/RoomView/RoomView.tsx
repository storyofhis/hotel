...
const RoomView: React.FC<TypeReservationStep> = (props: TypeReservationStep) => {
      ...
      roomTypeOptions.push({
        ...
        meta: (
          <>
            ...
            <span>{cart.days && cart.adults && getTotalPrice(type.price, +cart.days, +cart.adults).toFixed(2)} TL</span>
          </>
        ),
        imgUrl: type.photo,
      });
  ...
  if (selectedHotel?.details?.room_scenic.length) {
    selectedHotel.details.room_scenic.forEach((type: any) => {
  ...
  return (
    <Portlet>
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        <h4 className={[formClasses["form__label"], formClasses["form__section__title"]].join(" ")}>Room selection</h4>
        <div className={formClasses["form__wide-row"]}>
          <ImageCheckbox
            id="roomType"
        ...
        <h4 className={[formClasses["form__label"], formClasses["form__section__title"]].join(" ")}>View selection</h4>
        <div className={formClasses["form__wide-row"]}>
          <ImageCheckbox
            id="viewType"
