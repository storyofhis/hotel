...
const PreviewPayment: React.FC<TypeReservationStep> = (props: TypeReservationStep) => {
  ...
  useEffect(() => {
    if (!monthOptions.length) {
      setMonthOptions(
        getCreditCardMonths(formState.inputs.year.value).map((month: any) => ({
          label: month,
          value: month,
        }))
      );
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        <div className={styleClasses["preview-payment"]}>
          ...
          <div className={styleClasses["preview-payment__details"]}>
            <ReservationDetails type="boxes" show={["checkin", "checkout", "adults", "children", "room", "view", "coupon", "totals"]} />
          </div>
          <div className={styleClasses["preview-payment__form"]}>
            ...
            <div className={formClasses["form__wide-row"]}>
              {cardDetails.icon && <img className={styleClasses["credit-card-icon"]} src={cardDetails.icon} alt="Bank logo" />}
              <TextField
                id="cardNumber"
                type="text"
                label="Card number"
                ...
                onChange={(id: any, value: any, validity: any) => {
            ...
            <div className={formClasses["form__wide-row"]}>
              <div className={formClasses["form__have-two"]}>
                ...
                <Select
                  id="year"
                  options={getCreditCardYears().map((year: any) => ({
                    label: year.toString(),
                    value: year.toString(),
                  }))}
                  onChange={(id: any, value: any, validity: any) => {
                    ...
                    setMonthOptions(
                      getCreditCardMonths(value).map((month: any) => ({
                        label: month,
                        value: month,
                      }))
                    );
